...
5. **Second-project onboarding.** revnix repos need a fresh App installation (founder action); rext and client-a have no ledger yet, so intake there is founder-filing only. The registry and per-project specRepo fields are ready; nothing else is assumed.
