...
- **Product** (nextly, ranknaut, rext, ficonz, upkit): best foundation, then fast to market with
  ...
  ceiling.
- **Client** (client-a, client-b, client-b-console, client-a-2, client-c): deliver quality, best practices and
...
- **5 install toolchains**; 3 pnpm majors with mutually exclusive Node ranges. One repo
  (client-a) tracks **two divergent lockfiles** — `package-lock.json` resolves nextly alpha.20
...
- **5 repos have no CI.** client-b's `.github/workflows/` exists and is **empty**. rext's CI fires on
...
- Client-repo hazards, measured: **client-a's `next build` reaches a live production database**
  and sits in the same script block as `db:migrate:fresh`; **client-b auto-applies migrations on
...
- **4 profiles** written (nextly, rext-backend, client-a, reeve)
...
5. **Dogfood, rebuilt safely.** The old one was deleted from the hooks, not ported: it ran
   ...
   session start.
6. **Second project.** Prove the core/profile seam with rext-backend or client-a. Anything that
...
- `~/Work/Products/reeve/docs/2026-08-20-portfolio-readiness.md` — whether reeve can serve the other
  ...
  one repo; and outside nextly and client-org-c **no workflow triggers on a pull request**, so every PR
