...
| Date | Defect | Fix |
|---|---|---|
| 2026-08-29 | **A fixture's two outcomes were identical, so its test could not have failed.** The forged-line test asserts that a line produced by INTERLEAVING — stdout writing `FAIL  wrong assertion` unterminated, stderr writing ` the guard holds\n`, the two concatenating into an assertion neither stream emitted — cannot be classified. Whether that concatenation ever reaches the parent depends on the two stdout writes arriving as SEPARATE data events. Measured with and without the pauses between the writes: without them the writes coalesce into one data event, the forged line never forms, and the runner returns the SAME `WRONG_RED` and the SAME exit 1 as when it does. Both assertions stayed green either way, so the pass established nothing. Seventh instance in this lineage of a test passing for a reason other than the one it named, and found INSIDE the test written to close the sixth. The sweep cannot catch this class: a fixture that never exhibits the defect still goes red under a stub and reports `CAUGHT` exactly as a sound one would — the instrument's stated boundary, and where six of the seven have landed | A control spawns the same fixture body through the runner's own merge discipline — two async data handlers appending to one buffer in arrival order, because `spawnSync` hands back the streams already separated and could never observe an interleave — and fails loudly when the line is not forged. The body is written ONCE and shared, so the control cannot drift from the fixture it attests to. Its manifest entry stubs the FIXTURE rather than the runner, which is the only way to show that the control can go red; it is the single entry that does so and the reason is written beside it. The rule: a control must be able to FAIL, and a fixture whose two outcomes are indistinguishable is not a fixture |
