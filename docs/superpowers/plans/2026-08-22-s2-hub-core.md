...
// ── --takeover is advertised as the recovery, so it must recover ─────────────
{
  ...
  const refused = acquireSingleton(db, { name: "builder", pid: 400, lstart: "L400", command: "x", isAlive: NEVER });
  check(refused.ok === true, "a DEAD holder with an unexpired lease is takeable without the flag once liveness says dead");
...
export const COMPARISON_SET = [
  "task", "task_territory", "task_drain", "approval", "gate_request", "notice_receipt", "impl_pr", "attested_push",
...
export function restoreHub(snapshotPath, dbPath, { isAlive, pid, lstart, force = false } = {}) {
  ...
  try {
    ...
    if (tail.length) {
      ...
      try { replayed = replayHub(back, tail).applied; } finally { back.close(); }
    }
...
export function applyTransition(db, { taskId, expectedPhase, expectedGeneration, evidence,
  ...
  return hubTx(db, () => {
    ...
    if (decision.to === "CANCELLING") recordDrainSet(db, taskId, seq);
