...
CREATE UNIQUE INDEX IF NOT EXISTS provider_one_live_request
  ON provider_lease(owner, run_ref) WHERE status IN ('queued','held');
...
export const COMPARISON_SET = [
  "task", "task_territory", "approval", "gate_request", "notice_receipt", "impl_pr", "attested_push",
...
`);
  ...
  const probe = openHub(p);
  const seen = async () => {
    for (let i = 0; i < 200; i++) {
      const n = probe.prepare("SELECT count(*) c FROM phase_event").get().c;
      if (n >= 5 && n < 480) return n;         // moving, and not yet done
      await new Promise(r => setTimeout(r, 5));
    }
    return null;
  };
  const progress = await seen();
  check(progress !== null, "control: the child made observable progress before being killed",
    "if this is null the kill window was missed and the assertions below prove nothing");
  kid.kill("SIGKILL");
  await once(kid, "exit");
  probe.close();
...
```js
        ...
        // this dispatch site is `r`, the same one daemon.mjs:1222 already reads.
        // OUTCOMES is already imported in daemon.mjs (it is read at line 1222);
...
- Produces: `openHubAsGuest(path) -> DatabaseSync` — a connection whose `prepare`/`exec` refuse any statement outside the allowlist: `INSERT`/`UPDATE`/`DELETE`/`SELECT` on `provider_lease` and `provider_state`, `SELECT` on `pr_hold`, and the three transaction-control statements `BEGIN IMMEDIATE`, `COMMIT`, `ROLLBACK`. Everything else throws, naming the statement.
