...
export function applyCompensation(db, { c, taskId, generation, seq, evidence = {}, snapshot = null }) {
  ...
  switch (c) {
    ...
    case "release-territory": {
      // NO SPENT-PIN BOOKKEEPING HERE ANY MORE. The deadline lives on
      // `task_territory` beside the intent, so deleting a lease cannot separate
      // them and there is nothing for this path to preserve. The clear that used
      // to stand here, and its twin in `grantLease`, were both patches for a
