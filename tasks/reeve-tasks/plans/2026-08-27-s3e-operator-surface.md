...
| from | name | shape |
|---|---|---|
| S3-A T1 `src/build/capabilities.mjs` | the `builder.capabilities.*` reader | `capabilitiesFrom(profile) -> Readonly<Record<string, boolean>>` keyed by the **literal dotted strings `capabilityFor` emits** — `"builder.capabilities.observe"`, not `observe`. **The dotted form is the design, not an accident:** S3-A asserts its key set against `capabilityFor` by importing both sides (`src/build/outbox.mjs:306-323`), so the two cannot drift; a short-key map would be a second inventory of the same list. Read from the profile at call time. `WAITING_FOR_CAPABILITY` is derived from this **live** — Task 3 asserts the derivation flips with no write. MEASURED at `16cd880`: `git grep "capabilities" -- src bin` returns 23 hits and **none reads `observe`**; positive control, the same grep finds `mergeBuilderPr` read at `src/build/outbox.mjs:317`. So the reader genuinely does not exist yet. |
...
**Interfaces:**
- Consumes: `openHub` (S2-A `src/build/hubdb.mjs:322`); the `task`, `hold_reason`, `provider_lease`, `escalation`, `gate_request`, `notice_receipt`, `phase_run`, `task_territory` and `territory_lease` tables; `capabilitiesFrom(profile)` (S3-A T1, `src/build/capabilities.mjs`).
...
**Interfaces:**
- Consumes: `taskShow`, `taskList`, `renderShow`, `renderList`, `envelope`, `READ_FORMAT_VERSION` (Task 3); `whyModel`, `renderWhy` (Task 4); `fail`, `EXITS` (Task 2); `APPLIES` (Task 1); `hubPathFor` (`src/paths.mjs:69`); `capabilitiesFrom(profile)` (S3-A T1, `src/build/capabilities.mjs`).
