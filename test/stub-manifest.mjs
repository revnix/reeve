...
export const STUBS = [
  {
    name: "the-loaded-job-outranks-the-installed-file",
    why: "measure the checkout the plist names rather than the one launchd is running. launchd caches its configuration at bootstrap, so an edited plist does not reach a live job -- the file says what the NEXT load will use. Reading it as the running configuration lets the rule certify the provenance of a tree the daemon is not executing, which is the whole failure it exists to catch",
    test: "test/doctor-provenance.test.mjs",
    expectRed: "the LOADED arguments decide which checkout is measured",
    edits: [{ file: "src/doctor.mjs",
              find: "  const args = loaded ?? declared;",
              replace: "  const args = declared ?? loaded;" }],
  },
  {
    name: "a-process-on-another-commit-is-not-clean",
    why: "ignore what the RUNNING process loaded. A Node process holds the modules it read at startup, so pulling the checkout moves the tree and not the process: a clean current HEAD then certifies a daemon executing an older commit. Measured on this machine, the checkout was at e29cae6 while the live daemon had loaded a939cb1",
    test: "test/doctor-provenance.test.mjs",
    expectRed: "a running process on a DIFFERENT commit than HEAD is DEGRADED",
    edits: [{ file: "src/doctor.mjs",
              find: "  const started = runningCommit();",
              replace: "  const started = null;" }],
  },
  {
    name: "a-failed-status-probe-is-not-a-clean-tree",
    why: "treat an unreadable index as a clean tree. `rev-parse` and `rev-list` both succeed without a usable index, so falling through here returns OK having never established whether the tree on disk is the commit just named -- an absence read as success, in the one rule whose job is saying what is actually running",
    test: "test/doctor-provenance.test.mjs",
    expectRed: "a status probe that FAILED is UNKNOWN, not a clean tree",
    edits: [{ file: "src/doctor.mjs",
              find: "  if (!dirty.ok) return { id, level: UNKNOWN, title,",
              replace: "  if (false) return { id, level: UNKNOWN, title," }],
  },
  {
    name: "a-stale-tracking-ref-is-not-unreviewed-code",
    why: "compare against a local ref without checking it matches the remote. `rev-list origin/<branch>..HEAD` reads a tracking ref, so a checkout that has not fetched since its commit landed upstream is reported as carrying commits the branch has never seen. That is this rule's most alarming verdict -- the guardian is running unreviewed code -- delivered about a healthy deployment because of a stale ref",
    test: "test/doctor-provenance.test.mjs",
  ...
  {
    name: "a-plist-path-is-xml-decoded",
    why: "use a plist string node verbatim. A valid plist escapes `&`, so a launch argument like `/Users/A & B/deploy/bin/reeve` is stored as `&amp;` -- and running git against a literal `&amp;` directory reports BROKEN for a checkout that is perfectly healthy",
    test: "test/doctor-provenance.test.mjs",
    expectRed: "a plist string is XML-decoded before it is used as a path",
    edits: [{ file: "src/doctor.mjs",
              find: "  return String(text ?? \"\").replace(/&(?:amp|lt|gt|quot|apos);/g, (m) => XML_ENTITIES[m]);",
              replace: "  return String(text ?? \"\");" }],
  },
  {
    name: "a-symlinked-entry-point-resolves",
    why: "strip the textual path instead of resolving it. launchd may invoke a symlink such as `/usr/local/bin/reeve` whose target lives in the checkout; stripping gives `/usr/local`, and the rule then reports BROKEN because that is not a git repository -- about a deployment that is entirely correct",
    test: "test/doctor-provenance.test.mjs",
    expectRed: "a SYMLINKED entry point resolves to the checkout that holds the code",
    edits: [{ file: "src/doctor.mjs",
              find: "  try { resolved = realpath(bin); } catch { /* not on this machine; the textual path is all there is */ }",
              replace: "  resolved = bin;" }],
  },
  {
    name: "a-comparison-that-could-not-run-is-not-unreviewed-code",
    why: "report an unrunnable comparison as unreviewed code. `merge-base --is-ancestor` answers FALSE and FAILS with the same non-zero exit, which is why this rule counts twice instead -- and collapsing the two puts the most alarming verdict this rule has, `the guardian is running code no review saw`, on a git that simply could not run. An infrastructure failure wearing an authority failure's clothes is the one confusion that would make an operator distrust the rule itself",
    test: "test/doctor-provenance.test.mjs",
    expectRed: "a comparison that could not RUN is UNKNOWN, not unreviewed code",
    edits: [{ file: "src/doctor.mjs",
              find: "  if (!ahead.ok || !behind.ok) return { id, level: UNKNOWN, title,",
              replace: "  if (!ahead.ok || !behind.ok) return { id, level: BROKEN, title," }],
  },
  {
    name: "a-dirty-deploy-tree-is-broken",
    why: "accept a deploy tree with uncommitted changes. The whole question this rule answers is WHICH COMMIT the guardian runs, and a dirty tree has no answer: what executes is the promoted commit plus edits nobody recorded, so the reported sha is a claim about a file that is not there",
    test: "test/doctor-provenance.test.mjs",
    expectRed: "an uncommitted change in the deploy tree is BROKEN",
    edits: [{ file: "src/doctor.mjs",
              find: "  if (dirty.out) return { id, level: BROKEN, title,",
              replace: "  if (false) return { id, level: BROKEN, title," }],
  },
  {
    name: "code-not-on-the-default-branch-is-broken",
    why: "let the daemon run commits the default branch has never seen. That is the one state here that is genuinely dangerous rather than merely unclear: reeve enforces merge authority, review and CI on everyone else, from code that went through none of it",
    test: "test/doctor-provenance.test.mjs",
    expectRed: "a commit the default branch has never seen is BROKEN",
    edits: [{ file: "src/doctor.mjs",
              find: "  if (Number(ahead.out) > 0) return { id, level: BROKEN, title,",
              replace: "  if (false) return { id, level: BROKEN, title," }],
  },
  {
    name: "being-behind-is-not-a-fault",
    why: "degrade a guardian that is merely behind. A daemon held at a commit the founder promoted is working exactly as designed, and reporting that as a fault teaches the reader to skip this rule -- which then cannot deliver the verdict that matters. Distance is a number here, deliberately, and never a level",
    test: "test/doctor-provenance.test.mjs",
  ...
  {
    name: "provenance-does-not-claim-intent",
    why: "assert that a behind checkout was pinned on purpose. Nothing on the machine records a promotion, and the very checkout that produced this rule was 43 commits behind because nobody had pulled. A rule written to end that ambiguity must not resolve it by guessing the reassuring side",
    test: "test/doctor-provenance.test.mjs",
    expectRed: "does NOT assert the distance was deliberate",
    edits: [{ file: "src/doctor.mjs",
              find: "              : `which is ${behind.out} commit(s) behind origin/${defaultBranch}`,",
              replace: "              : `which is ${behind.out} commit(s) behind origin/${defaultBranch} -- pinned, not drifting`," }],
  },
  {
  ...
  {
    name: "the-bin-reeve-match-is-anchored",
    why: "match any argument merely CONTAINING reeve. The rule derives the deploy checkout by stripping `/bin/reeve` from an argument, so an unanchored match would take a path like `/usr/bin/reeved`, strip nothing, and then run git against a directory that has nothing to do with the daemon -- reporting a commit for the wrong tree entirely",
    test: "test/doctor-provenance.test.mjs",
    expectRed: "a path merely CONTAINING reeve is not bin/reeve",
    edits: [{ file: "src/doctor.mjs",
              find: "  const bin = (args ?? []).find(a => typeof a === \"string\" && /(^|\\/)bin\\/reeve$/.test(a));",
              replace: "  const bin = (args ?? []).find(a => typeof a === \"string\" && /reeve/.test(a));" }],
  },
  {
    name: "lease-gate",
