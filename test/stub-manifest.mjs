...
export const STUBS = [
  ...
  },
  {
    ...
    test: "test/doctor-provenance.test.mjs",
    expectRed: "a job loaded with NO running process is DEGRADED, not OK",
    edits: [{ file: "src/doctor.mjs",
              find: "  if (pid === null) return { id, level: DEGRADED, title,",
              replace: "  if (false) return { id, level: DEGRADED, title," }],
  },
  {
    name: "the-startup-record-must-be-this-processs",
    why: "accept a startup record written by a different process. The daemon log is shared -- a manually started daemon writes to it too, and a job restarted since the last line has an older record above its own -- so without matching the pid launchctl reports, this rule describes whatever wrote last",
    test: "test/doctor-provenance.test.mjs",
    expectRed: "a startup record from ANOTHER pid says nothing about this job",
    edits: [{ file: "src/doctor.mjs",
              find: "  if (record?.pid !== undefined && record.pid !== pid) return { id, level: UNKNOWN, title,",
              replace: "  if (false) return { id, level: UNKNOWN, title," }],
  },
  {
    name: "an-absent-tree-record-cannot-end-in-ok",
    why: "annotate a record with no tree state and pass anyway. That record was written before the field existed, so it never established whether the process loaded uncommitted code -- the exact fact the field was added for. Passing while saying so asserts the reassuring half of something nobody measured",
    test: "test/doctor-provenance.test.mjs",
    expectRed: "a record with NO tree state is DEGRADED, not OK",
    edits: [{ file: "src/doctor.mjs",
              find: "  if (record.tree === null) return { id, level: DEGRADED, title,",
              replace: "  if (false) return { id, level: DEGRADED, title," }],
  },
  {
    name: "the-home-flag-outranks-the-environment",
    why: "read REEVE_HOME when the job was started with --home. `bin/reeve` derives HOME from the flag first, so a job using `--home /custom` without `--log` writes /custom/reeve.log -- and reading the environment instead finds a stale record from another invocation, or none, on which this rule refuses about a healthy daemon",
    test: "test/doctor-provenance.test.mjs",
    expectRed: "--home OUTRANKS the environment",
    edits: [{ file: "src/doctor.mjs",
              find: "  const h = (args ?? []).indexOf(\"--home\");",
              replace: "  const h = -1;" }],
  },
  {
    name: "untracked-files-count-as-a-dirty-tree",
    why: "let `status.showUntrackedFiles=no` hide new source files. A checkout carrying that configuration reports a tree with uncommitted files in it as clean, and a committed module importing one of them executes code no commit contains -- which the startup record then states as a clean tree, with R-17 passing it on",
    test: "test/doctor-provenance.test.mjs",
  ...
  },
  {
    ...
    why: "let the installed plist stand in for a loaded job. launchd caches its configuration at bootstrap, so the file describes the NEXT load and says nothing about whether a daemon exists -- reading it alone lets this rule report a healthy guardian on a machine where none is running, which is the one claim it must never make wrongly",
    test: "test/doctor-provenance.test.mjs",
    expectRed: "a readable plist with NO loaded job is UNKNOWN, not OK",
    edits: [{ file: "src/doctor.mjs",
              find: "  if (loaded === null) {",
              replace: "  if (false) {" }],
  },
  {
    name: "an-unreadable-startup-is-not-healthy",
    why: "return OK when no startup record could be read. The rule would then exit healthy while admitting in its own output that it cannot say what the running process loaded -- which is the only question it exists to answer, so a pass there is worse than no rule",
    test: "test/doctor-provenance.test.mjs",
    expectRed: "no readable startup record is UNKNOWN, not OK",
    edits: [{ file: "src/doctor.mjs",
              find: "  if (record === null) return { id, level: UNKNOWN, title,",
              replace: "  if (false) return { id, level: UNKNOWN, title," }],
  },
  {
    name: "a-dirty-startup-tree-is-recorded-not-inferred",
    why: "ignore the tree state the daemon recorded at startup. Reading `git status` now answers a question about a DIFFERENT moment: an edit made after startup is a false alarm, and a daemon started from a dirty tree since reverted reads clean while the process still holds uncommitted code. Neither is recoverable afterwards, which is why the daemon records it",
    test: "test/doctor-provenance.test.mjs",
    expectRed: "a process that loaded from a DIRTY tree is BROKEN",
    edits: [{ file: "src/doctor.mjs",
              find: "  if (record.tree === \"dirty\") return { id, level: BROKEN, title,",
              replace: "  if (false) return { id, level: BROKEN, title," }],
  },
  {
    name: "the-running-commit-is-judged-before-the-divergence",
    why: "report a checkout that has moved before asking whether the RUNNING commit is on the branch. Ordering is the whole finding: a daemon executing an unreviewed commit escapes this rule's BROKEN verdict the moment someone moves the checkout to a reviewed HEAD, because the divergence is noticed first and DEGRADED is returned",
    test: "test/doctor-provenance.test.mjs",
    expectRed: "a RUNNING commit the branch has never seen is BROKEN even when the checkout moved",
    edits: [{ file: "src/doctor.mjs",
              find: "  if (Number(aheadOfRunning.out) > 0) return { id, level: BROKEN, title,",
              replace: "  if (false) return { id, level: BROKEN, title," }],
  },
  {
    name: "a-stale-tracking-ref-is-not-unreviewed-code",
    why: "compare against a local tracking ref without checking it matches the remote. A checkout that has not fetched since its commit landed upstream is then reported as running commits the branch has never seen -- this rule's most alarming verdict, delivered about a healthy deployment because of a stale ref",
    test: "test/doctor-provenance.test.mjs",
    expectRed: "a STALE tracking ref is UNKNOWN rather than unreviewed code",
    edits: [{ file: "src/doctor.mjs",
              find: "  if (localRef.out !== live) return { id, level: UNKNOWN, title,",
              replace: "  if (false) return { id, level: UNKNOWN, title," }],
  },
  {
    name: "a-comparison-that-could-not-run-is-not-unreviewed-code",
    why: "report an unrunnable comparison as unreviewed code. `merge-base --is-ancestor` answers FALSE and FAILS with the same non-zero exit, which is why this counts instead -- collapsing them puts `the guardian is running code no review saw` on a git that simply could not run",
    test: "test/doctor-provenance.test.mjs",
    expectRed: "a comparison that could not RUN is UNKNOWN, not unreviewed code",
    edits: [{ file: "src/doctor.mjs",
              find: "  if (!aheadOfRunning.ok) return { id, level: UNKNOWN, title,",
              replace: "  if (false) return { id, level: UNKNOWN, title," }],
  },
  {
    name: "being-behind-is-not-a-fault",
    why: "degrade a guardian that is merely behind. A daemon held at a commit the founder deployed is working as designed, and reporting that as a fault teaches the reader to skip this rule -- which then cannot deliver the verdict that matters",
    test: "test/doctor-provenance.test.mjs",
    expectRed: "behind the default branch is still OK, because pinning is legitimate",
    edits: [{ file: "src/doctor.mjs",
              find: "  return { id, level: OK, title,\n    lines: [...lines,",
              replace: "  return { id, level: Number(behind.out) > 0 ? DEGRADED : OK, title,\n    lines: [...lines," }],
  },
  {
    name: "an-absent-tree-record-is-not-clean",
    why: "read a record with no tree field as a clean tree. Older daemons wrote no tree state at all, so treating absent as clean silently asserts the reassuring half of a fact that was never recorded -- the same defect as calling an unrecorded HEAD a promoted commit, one field over",
    test: "test/doctor-provenance.test.mjs",
    expectRed: "an absent tree record says so instead of being read as clean",
    edits: [{ file: "src/doctor.mjs",
              find: "  return { pid: Number(pid), commit: commit === \"unreadable\" ? null : commit,\n           tree: tree === undefined || tree === \"unreadable\" ? null : tree };",
              replace: "  return { pid: Number(pid), commit: commit === \"unreadable\" ? null : commit,\n           tree: tree ?? \"clean\" };" }],
  },
  {
  ...
  },
  {
    ...
    why: "resolve a relative entry point from wherever doctor happens to run. `node bin/reeve` with a WorkingDirectory is a valid job, and resolving it against the caller's cwd finds a different tree or none -- so the rule reports on a checkout that has nothing to do with the daemon",
    test: "test/doctor-provenance.test.mjs",
    expectRed: "a RELATIVE entry point resolves against the job's working directory",
    edits: [{ file: "src/doctor.mjs",
              find: "  const absolute = bin.startsWith(\"/\") ? bin : (cwd ? join(cwd, bin) : null);",
              replace: "  const absolute = bin;" }],
  },
  {
    name: "the-daemon-log-path-comes-from-the-job",
    why: "read doctor's own home instead of the job's. `bin/reeve` resolves the log as `opt(\"log\") ?? join(HOME, \"reeve.log\")`, so a job using --log or a different REEVE_HOME writes elsewhere -- and this rule would then compare HEAD against a stale record from another invocation, or find none and refuse on a healthy daemon",
    test: "test/doctor-provenance.test.mjs",
    expectRed: "the --log argument decides where the daemon writes",
    edits: [{ file: "src/doctor.mjs",
              find: "  const i = (args ?? []).indexOf(\"--log\");\n  if (i >= 0 && args[i + 1]) return args[i + 1];",
              replace: "  const i = -1;\n  if (i >= 0 && args[i + 1]) return args[i + 1];" }],
  },
  {
    name: "a-symlinked-entry-point-resolves",
    why: "strip the textual path instead of resolving it. launchd may invoke a symlink such as `/usr/local/bin/reeve` whose target lives in the checkout; stripping gives `/usr/local`, and the rule reports BROKEN about a deployment that is entirely correct",
    test: "test/doctor-provenance.test.mjs",
    expectRed: "a SYMLINKED entry point resolves to the checkout that holds the code",
    edits: [{ file: "src/doctor.mjs",
              find: "  try { resolved = realpath(absolute); } catch { /* not on this machine; the path is all there is */ }",
              replace: "  resolved = absolute;" }],
  },
  {
    name: "lease-gate",
