...
export const STUBS = [
  ...
  },
  {
    ...
    edits: [{ file: "scripts/state.mjs",
              find: "total === 0 || caught !== total || entries !== total",
              replace: "total === 0 || caught !== total" }],
  },
  {
    ...
    test: "test/state-script.test.mjs",
    expectRed: "runner keeps the WHOLE stderr diagnostic, not only its last line",
    edits: [{ file: "scripts/state.mjs",
              find: "               err: (all.length ? all.join(\" | \") : \"failed\").slice(0, 1200) };",
              replace: "               err: (all.pop() ?? \"failed\").slice(0, 1200) };" }],
  },
  {
    ...
    test: "test/state-script.test.mjs",
    expectRed: "an `unreadable` commit is a RECORD, not a miss",
    edits: [{ file: "scripts/state.mjs",
              find: "  return { pid: Number(pid), commit: commit === \"unreadable\" ? null : commit };",
              replace: "  return commit === \"unreadable\" ? null : { pid: Number(pid), commit };" }],
  },
  {
    name: "the-startup-record-is-anchored-to-the-whole-line",
    why: "match `running commit <sha>` anywhere instead of the daemon's whole startup line. The log is shared and its later lines carry externally influenced text -- a failing GitHub check named `running commit abcdef1` reaches it through `describe()` -- so an unanchored pattern lets a check name decide what the guardian is reported to be running",
    test: "test/state-script.test.mjs",
  ...
  },
  {
    ...
    why: "accept a sweep that exited zero having printed nothing. A build that produced no output, or an early return, would then give `ok` with a blank line and let the caller continue -- the exact absence read as success that this script exists to prevent, in the function written to prevent it",
    test: "test/state-script.test.mjs",
  ...
  },
  {
    ...
    why: "name missing dependencies as the cause of every UNRUNNABLE. `stubsweep` also emits it when the control test is already red on the base, when a restore fails, and when a test leaves side effects -- so a single-cause remedy sends the operator to the wrong place three times out of four, and the one time it is right they would have found it anyway",
    test: "test/state-script.test.mjs",
    expectRed: "it does not name dependencies as THE cause",
    edits: [{ file: "scripts/state.mjs",
              find: "            \"causes differ: a control already red on the base, a failed restore, a test with side effects, or missing dependencies\",",
              replace: "            \"check dependencies are installed in the sweep tree\"," }],
  },
  {
  ...
  },
  {
    ...
    why: "report an entry the sweep could not RUN as an assertion that cannot fail. Those are unrelated facts: one is about this machine, the other is the finding the sweep exists to produce. Conflating them is exactly what sent a session hunting a defect in a healthy repository -- test/lint-no-url-pathname.test.mjs imports eslint, the sweep worktree had no dependencies installed, and main's CI was green on the very commit the report called broken",
    test: "test/state-script.test.mjs",
    expectRed: "an entry that could not RUN is a fact about this machine",
    edits: [{ file: "scripts/state.mjs",
              find: "  if (unrunnable.length) return { level: \"environment\",",
              replace: "  if (unrunnable.length) return { level: \"refusal\"," }],
  },
  {
    name: "a-real-finding-outranks-an-environment-note",
    why: "stop reporting a genuine NOT_CAUGHT when an unrunnable entry appears beside it. An assertion that cannot fail is true regardless of what else the machine could not run, so a missing dependency must never demote a real finding into a note about dependencies -- which is the same defect as the entry above, in the direction that loses information rather than the one that invents it",
    test: "test/state-script.test.mjs",
    expectRed: "control: a real finding alongside an unrunnable entry is still a refusal",
    edits: [{ file: "scripts/state.mjs",
              find: "  if (uncaught.length) return { level: \"refusal\",",
              replace: "  if (false) return { level: \"refusal\"," }],
  },
  {
