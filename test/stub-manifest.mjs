...
`,
  ...
  },
  {
    ...
    edits: [{ file: "src/pr.mjs",
              find: "    if (p && typeof p === \"object\") byProtection = (p.contexts ?? []).includes(context) || (p.checks ?? []).some((c) => c?.context === context);",
              replace: "    if (p && typeof p === \"object\") byProtection = false;" }],
  },
  {
    name: "shadow-403-is-not-404",
    why: "read any failure to read branch protection as an unprotected branch. A 403 then passes as not required, and the gate opens",
    test: "test/shadow-required-check.test.mjs",
    expectRed: "nor when branch protection can't be read (a 403, unlike the 404 of an unprotected branch)",
    edits: [{ file: "src/pr.mjs",
              find: "  } else if (/\\(HTTP 404\\)/.test(protection?.err ?? \"\")) byProtection = false;",
              replace: "  } else if (/\\(HTTP \\d+\\)/.test(protection?.err ?? \"\")) byProtection = false;" }],
  },
  {
