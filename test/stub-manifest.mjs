...
export const STUBS = [
  ...
  },
  {
    ...
    test: "test/state-script.test.mjs",
    expectRed: "an empty log answers null rather than an empty string",
    edits: [{ file: "scripts/state.mjs",
              find: "  return hits.length ? hits[hits.length - 1][1] : null;",
              replace: "  return hits.length ? hits[hits.length - 1][1] : \"\";" }],
  },
  {
    name: "the-last-daemon-start-wins",
    why: "read the FIRST recorded start rather than the last. The log is append-only across restarts, so the first line names a process that has since been replaced -- and the whole reason this fact is measured is that a running daemon holds the modules it loaded, which is only true of the start that is still running",
    test: "test/state-script.test.mjs",
