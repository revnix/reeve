...
`,
  ...
  {
    name: "shadow-required-does-not-pass",
    why: "publish neutral in shadow mode whatever the rules say. With ops/merge-policy required, every pull request then passes the gate unjudged",
    test: "test/shadow-required-check.test.mjs",
    expectRed: "with the check required by a ruleset, shadow mode publishes a conclusion that doesn't pass, and says why",
    edits: [{ file: "src/pr.mjs",
              find: "  return required === false ? \"neutral\" : \"action_required\";",
              replace: "  return \"neutral\";" }],
  },
  {
    name: "shadow-unknown-is-not-not-required",
    why: "take rules that couldn't be read as not requiring the check. A token without permission to read them then reopens the gate in shadow mode",
    test: "test/shadow-required-check.test.mjs",
    expectRed: "when the rules can't be read, shadow mode doesn't pass: unknown is never taken for not required",
    edits: [{ file: "src/pr.mjs",
              find: "  return required === false ? \"neutral\" : \"action_required\";",
              replace: "  return required !== true ? \"neutral\" : \"action_required\";" }],
  },
  {
    name: "shadow-reads-classic-protection",
    why: "read only rulesets. A check required by classic branch protection then still passes in shadow mode",
    test: "test/shadow-required-check.test.mjs",
    expectRed: "with the check required by classic branch protection, it doesn't pass either",
    edits: [{ file: "src/pr.mjs",
              find: "      byProtection = (checks.contexts ?? []).includes(context) || (checks.checks ?? []).some((c) => c?.context === context);",
              replace: "      byProtection = false;" }],
  },
  {
    name: "shadow-unreadable-branch-is-unknown",
    why: "read a branch that couldn't be read as unprotected. A token without access then passes as not required, and the gate opens",
    test: "test/shadow-required-check.test.mjs",
    expectRed: "nor when the branch's own protection can't be read",
    edits: [{ file: "src/pr.mjs",
              find: "  if (byRules === true || byProtection === true) return true;",
              replace: "  if (!branch?.ok) byProtection = false;\n  if (byRules === true || byProtection === true) return true;" }],
  },
  {
    name: "shadow-context-matches-exactly",
    why: "count any required check as this one. Shadow mode then refuses to publish neutral on every protected branch, and the trial's checks all read as failures",
    test: "test/shadow-required-check.test.mjs",
    expectRed: "control: another check being required doesn't make this one required",
    edits: [{ file: "src/pr.mjs",
              find: "      && (r.parameters?.required_status_checks ?? []).some((c) => c?.context === context));",
              replace: "      && (r.parameters?.required_status_checks ?? []).length > 0);" }],
  },
  {
    name: "daemon-passes-the-base",
    why: "publish without the pull request's base branch. The publisher then can't read the base's rules, and every shadow check is held back as unknown",
    test: "test/shadow-required-check.test.mjs",
    expectRed: "the daemon tells the publisher which branch the pull request targets",
    edits: [{ file: "src/daemon.mjs",
              find: "({ nwo, verdict: e.verdict, shadow, base: e.baseRef });",
              replace: "({ nwo, verdict: e.verdict, shadow });" }],
  },
  {
    name: "daemon-raises-held-shadow",
    why: "log a held-back shadow check without raising it. The person then learns only from a log that the gate needs a decision",
    test: "test/shadow-required-check.test.mjs",
    expectRed: "and a check shadow mode held back is logged and raised once for the person, not once per pull request",
    edits: [{ file: "src/daemon.mjs",
              find: "      raise(`shadow mode can't publish a passing-neutral check: ${pub.held}`);\n",
              replace: "" }],
  },
  {
