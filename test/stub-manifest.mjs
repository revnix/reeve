...
`,
  ...
  },
  {
    ...
    edits: [{ file: "src/profile/detect.mjs",
              find: "  if (!first || /^[({]/.test(first) || SHELL_WORDS.has(first)) return null;\n",
  ...
  },
  {
    ...
    edits: [{ file: "src/profile/detect.mjs",
              find: "  while (words.length && /^[A-Za-z_][A-Za-z0-9_]*=/.test(words[0])) {\n    if ((words[0].match(/[\"']/g) ?? []).length % 2) return null;\n    words.shift();\n  }\n",
              replace: "" }],
  },
  {
    name: "detect-quoted-assignment-not-misread",
    why: "split a quoted assignment value on its spaces. The second half of the value is then read as the program, and a working script is reported broken",
    test: "test/detect-shell-scripts.test.mjs",
    expectRed: "an assignment whose quoted value spans words is not misread as a program",
    edits: [{ file: "src/profile/detect.mjs",
              find: "    if ((words[0].match(/[\"']/g) ?? []).length % 2) return null;\n",
              replace: "" }],
  },
  {
