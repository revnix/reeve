...
`,
  ...
  },
  {
    ...
    edits: [{ file: "src/build/artifact.mjs",
              find: "      if (CLAIM.test(line) && !CITATION.test(line)) findings.push(`no file:line citation: ${line.trim()}`);",
              replace: "      if (!CITATION.test(text)) findings.push(`no file:line citation: ${line.trim()}`);" }],
  },
  {
