...
`,
  ...
  },
  {
    ...
    why: "read no canary out of the containment verdict, which is the state this change found. The founder's page list names three identities that may interrupt a human; `builder:sandbox:canary-failed` existed only inside a comment in `build/dash.mjs` and no site minted it, so the one condition under which NOTHING may dispatch reached the daily digest and never a phone",
  ...
  {
    name: "canary-page-names-which-canary",
    why: "page with the bare identity and no canary id. `buildAlert` renders the escalation KEY as the message body, and the key is also the dedup identity -- so without the id a sandbox that fails, is repaired, and fails again under a NEW policy is the same key as before, and `announceable` treats the second fault as one it has already announced",
    test: "test/canary-page.test.mjs",
    expectRed: "the page names WHICH canary failed",
    edits: [{ file: "src/daemon.mjs",
              find: "    raise(`${CANARY_PAGE}: ${canary.id}`);",
              replace: "    raise(CANARY_PAGE);" }],
  },
  {
    name: "canary-page-stands-across-a-quiet-tick",
    why: "stop re-raising the pages the store already carries. Containment is measured only when a worker task wants dispatching, so a quiet tick runs no canary -- and `announceable` reads an escalation absent from a complete tick as resolved, so the next quiet tick announces CLEARED for a sandbox nobody re-measured",
    test: "test/canary-page.test.mjs",
    expectRed: "the page still stands on a second tick",
    edits: [{ file: "src/daemon.mjs",
              find: "    for (const why of standingCanaryPages(db)) raise(why);",
              replace: "    for (const why of []) raise(why);" }],
  },
  {
    name: "canary-page-survives-a-restart",
    why: "answer the standing-page query with nothing, which is what a latch kept in this process's memory amounts to across a restart. `bin/reeve` builds a fresh context per process, so a daemon restart or a one-shot `reeve tick` arrives with an empty latch while the escalation ROW is still in the store -- and the next complete tick finds that row standing and absent and retires it. A restart would announce that a sandbox nobody re-measured is fine. This is the shape review found in the first version of this change",
    test: "test/canary-page.test.mjs",
    expectRed: "the page survives a RESTART",
    edits: [{ file: "src/daemon.mjs",
              find: "    .all(CANARY_PAGE.length, CANARY_PAGE).map(r => r.why);",
              replace: "    .all(CANARY_PAGE.length, CANARY_PAGE).map(r => r.why).slice(0, 0);" }],
  },
  {
    name: "canary-page-clears-when-one-passes",
    why: "re-raise the standing pages even on a tick whose canary RAN. The page then becomes a latch that only a restart can silence, and an operator who is never told a fault is over cannot tell `resolved` from `reeve stopped looking` -- which is the reason `announceable` announces clearing at all. A canary that ran supersedes the standing set: the id names the configuration in force, so a page about one no longer running is not actionable",
    test: "test/canary-page.test.mjs",
  ...
  },
  {
    ...
    why: "drop the canary's own reason from the log line. The reason is deliberately NOT in the escalation key -- it moves between runs of one broken sandbox, and a key that moves is retired and re-raised, which is two pushes for one unchanged fault -- so the log is the only place it survives. Without it the page says a canary failed and nothing anywhere says why",
    test: "test/canary-page.test.mjs",
