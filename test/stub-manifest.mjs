...
`,
  ...
  },
  {
    ...
    why: "take a skipped check for one nothing requires when the base's requirements couldn't be read. A required job that was skipped then passes",
    test: "test/ci-read-completely.test.mjs",
    expectRed: "where the base's requirements couldn't be read, a skipped or neutral check may be a required one, so it isn't green",
    edits: [{ file: "src/github/reconciler.mjs",
              find: "  const unplaced = requiredKnown || !evidence ? [] :",
              replace: "  const unplaced = true ? [] :" }],
  },
  {
  ...
  },
  {
    ...
    test: "test/ci-read-completely.test.mjs",
    expectRed: "a required check bound to an App is met only by that App's runs: another App's pass isn't it, and a status can't be told",
    edits: [{ file: "src/github/reconciler.mjs",
