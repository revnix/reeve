...
`,
  ...
  },
  {
    ...
    why: "accept an OPENING fence as a done condition. Half a block is not a done condition, and the rest of the document is then inside it -- so what the slice actually asks for is whatever happens to follow, which no reader would guess from a gate that reported ok",
    test: "test/artifact.test.mjs",
    expectRed: "an unterminated done-condition fence is refused",
    edits: [{ file: "src/build/artifact.mjs",
              find: "          const fence = /^[ \\t]*(`{3,})[^`\\n]*\\n[ \\t]*(\\S[^\\n]*)[\\s\\S]*?^[ \\t]*\\1`*[ \\t]*$/m.exec(within);",
              replace: "          const fence = /^[ \\t]*(`{3,})[^`\\n]*\\n[ \\t]*(\\S[^\\n]*)/m.exec(within);" }],
  },
  {
  ...
  },
  {
    ...
    why: "accept a closing fence shorter than the one that opened the block. A done condition opened with four backticks and closed with three is NOT closed -- CommonMark requires the closer to be at least as long -- so the whole remainder of design.md is inside the fence while this gate reports a machine-checkable done condition. The gate and the renderer then disagree about where the slice ends, and the gate is the one that says the work may proceed",
    test: "test/artifact.test.mjs",
