...
`,
  ...
  },
  {
    ...
    test: "test/process-identity.test.mjs",
    expectRed: "a token recorded before the pin, in another timezone, still identifies its process",
    edits: [{ file: "src/supervisor.mjs",
              find: "  return oldTokenNames(storedStart, now);",
              replace: "  return false;" }],
  },
  {
    name: "process-identity-old-tokens-need-a-real-offset",
    why: "accept an old token within any distance of the true start. A stranger that reused the pid and started hours later then reads as the process that was recorded, and could be reaped or trusted as it",
    test: "test/process-identity.test.mjs",
    expectRed: "an old token off by anything but a real timezone's offset names a different process",
    edits: [{ file: "src/supervisor.mjs",
              find: "  return diff % 60000 === 0 && offsetsInUseAt(start).has(diff / 60000);",
