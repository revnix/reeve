...
`,
  ...
  },
  {
    ...
    edits: [{ file: "src/pr.mjs",
              find: "    if (stale && stale.app === POLICY_APP && [\"success\", \"neutral\", \"skipped\"].includes(stale.conclusion)) {",
              replace: "    if (false) {" }],
  },
  {
  ...
  },
  {
    ...
    edits: [{ file: "src/pr.mjs",
              find: "      .flatMap((r) => r.parameters?.required_status_checks ?? []).filter((c) => c?.context === context).map((c) => ours(c.integration_id)));",
              replace: "      .flatMap((r) => r.parameters?.required_status_checks ?? []).map((c) => ours(c.integration_id)));" }],
  },
  {
    ...
    edits: [{ file: "src/pr.mjs",
              find: "      byProtection = verdictOf([...(checks.checks ?? []).filter((c) => c?.context === context).map((c) => ours(c.app_id)),",
              replace: "      byProtection = false; void verdictOf([" }],
  },
  {
    ...
    edits: [{ file: "src/pr.mjs",
              find: "  if (byRules === null || byProtection === null) return null;\n",
              replace: "" }],
  },
  {
  ...
  },
  {
    ...
    edits: [{ file: "src/verdict.mjs",
              find: "    else add(\"mergeable\", PASS, \"mergeStateStatus BLOCKED by reeve's own required check, which this verdict decides; no conflict, and no review outstanding\");",
              replace: "    else add(\"mergeable\", BLOCK, \"mergeStateStatus BLOCKED by reeve's own required check, which this verdict decides; no conflict, and no review outstanding\");" }],
  },
  {
  ...
  },
  {
    ...
    why: "never read whether reeve's check is required. BLOCKED is then always unknown, and a required check still can't reach PASS",
    test: "test/own-check-blocked.test.mjs",
    expectRed: "when BLOCKED, the parts read whether reeve's check is required on the base, from its rules and its protection",
    edits: [{ file: "src/pr.mjs",
              find: "  if (String(threads?.mergeState ?? \"\").toUpperCase() === \"BLOCKED\" && baseRef)",
              replace: "  if (false)" }],
  },
  {
