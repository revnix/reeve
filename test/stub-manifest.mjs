...
`,
  ...
  },
  {
    ...
    edits: [{ file: "src/pr.mjs",
              find: "  const all = [...(profile.ci?.requiredChecks ?? []).map((context) => ({ context, app: null })), ...(base ?? []).filter((c) => !aside.has(c.context))];",
              replace: "  const all = [...(profile.ci?.requiredChecks ?? []).map((context) => ({ context, app: null }))];" }],
  },
  {
