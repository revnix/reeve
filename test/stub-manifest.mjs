...
`,
  ...
  },
  {
    ...
    why: "map anything that is not `infeasible` or `blocked` to `phase.succeeded`, so a report that skipped validation, or one validated against a different action, ADVANCES the task on a word nobody declared. Failing open is the wrong direction for the one function whose output moves a task forward",
    test: "test/phase-report.test.mjs",
    expectRed: "an outcome of \"nonsense\" is refused rather than advanced",
    edits: [{ file: "src/build/report.mjs",
              find: "  if (!declared.includes(report?.outcome))",
              replace: "  if (false && !declared.includes(report?.outcome))" }],
  },
  {
