...
`,
  ...
  },
  {
    ...
    edits: [{ file: "src/github/reconciler.mjs",
              find: "  if (skipped.length) return { verdict: \"MISSING_REQUIRED\",",
              replace: "  if (false) return { verdict: \"MISSING_REQUIRED\"," }],
  },
  {
  ...
  },
  {
    ...
    edits: [{ file: "src/github/reconciler.mjs",
              ...
              replace: "  if (failing.length && !requiredChecks.some(c => names.has(c) && notRun(c))) return { verdict: \"RED\"," }],
  },
  {
  ...
  },
  {
    ...
    edits: [{ file: "src/pr.mjs",
              find: "  const every = Array.isArray(list) && b ? [...new Set(names)] : null;",
              replace: "  const every = whole ? [...new Set(names)] : null;" }],
  },
  {
    ...
    edits: [{ file: "src/pr.mjs",
              find: "  const required = [...new Set([...(profile.ci?.requiredChecks ?? []), ...(base ?? []).filter((n) => !aside.has(n))])];",
              replace: "  const required = [...new Set(profile.ci?.requiredChecks ?? [])];" }],
  },
  {
    ...
    test: "test/ci-read-completely.test.mjs",
    expectRed: "a read that isn't whole classifies as unknown, whatever the part that was read says",
    edits: [{ file: "src/pr.mjs",
              find: "  if (!read?.ok) return { verdict: \"UNKNOWN\"",
  ...
  },
  {
    ...
    edits: [{ file: "src/github/reconciler.mjs",
              find: "  const skipped = evidence ? requiredChecks.filter(c => names.has(c) && notRun(c)) : [];",
              replace: "  const skipped = requiredChecks.filter(c => names.has(c) && notRun(c));" }],
  },
  {
    ...
    expectRed: "a pull request's required checks are the profile's and the base's, reeve's own and reviewers' statuses aside",
    edits: [{ file: "src/pr.mjs",
              find: "shadowContextOf(POLICY_CONTEXT), ...(profile.ci?.reviewerStatusContexts ?? [])]);",
              replace: "shadowContextOf(POLICY_CONTEXT)]);" }],
  },
];
