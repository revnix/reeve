...
`,
  ...
  {
    name: "premerge-unreviewed-is-not-clear",
    why: "let an empty thread list mean the review was clear, when it also means nobody looked",
    test: "test/premerge.test.mjs",
    expectRed: "no threads at all is UNREVIEWED, not clear",
    edits: [{ file: "src/premerge.mjs",
              find: "  if (totalCount === 0)",
              replace: "  if (false)" }],
  },
  {
  ...
  },
  {
    ...
    test: "test/premerge.test.mjs",
    expectRed: "an unreviewed pull request is not clear even with a current head",
    edits: [{ file: "src/premerge.mjs",
