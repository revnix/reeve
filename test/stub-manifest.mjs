...
`,
  ...
  },
  {
    ...
    edits: [{ file: "src/paths.mjs",
              find: "  return `no state database at ${dbPath}` + (initCreatesIt ? \"\\n-> reeve init --write   creates it\" : \"\");",
  ...
  },
  {
    ...
    edits: [{ file: "bin/reeve",
              find: "    if (!existsSync(dbPath)) { console.error(`reeve backup: ${missingStoreMessage(dbPath, { initCreatesIt: !opt(\"db\") })}`); process.exit(1); }",
  ...
  },
  {
    ...
    expectRed: "every command that needs the state database names the step that creates it",
    edits: [{ file: "bin/reeve",
              find: "    if (!existsSync(dbPath)) { console.error(`reeve shadow: ${missingStoreMessage(dbPath, { initCreatesIt: !opt(\"db\") })}`); process.exit(1); }",
  ...
  },
  {
    ...
    expectRed: "every command that needs the state database names the step that creates it",
    edits: [{ file: "bin/reeve",
              find: "      fail(\"store_absent\", dbPath ? `reeve ${cmd}: ${missingStoreMessage(dbPath, { initCreatesIt: !opt(\"db\") })}` : `reeve ${cmd}: no state database`);",
  ...
  },
  {
    ...
    edits: [{ file: "src/init.mjs",
              find: "      : { changed: false, line: said ?? `could not move the legacy store; using ${used}` };",
              replace: "      : { changed: false, line: `could not move the legacy store; using ${used}` };" }],
  },
];
