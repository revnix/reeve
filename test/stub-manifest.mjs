...
`,
  ...
  },
  {
    ...
    why: "page with the bare identity and no canary id. `buildAlert` renders the escalation KEY as the message body, and the key is also the dedup identity -- so without the id a sandbox that fails, is repaired, and fails again under a NEW policy is the same key as before, and `announceable` treats the second fault as an already-announced one",
    test: "test/canary-page.test.mjs",
    expectRed: "the page names WHICH canary failed",
    edits: [{ file: "src/daemon.mjs",
              find: "    ctx.canaryFailure = `builder:sandbox:canary-failed: ${canary.id}`;",
              replace: "    ctx.canaryFailure = `builder:sandbox:canary-failed`;" }],
  },
  {
    name: "canary-page-stands-across-a-quiet-tick",
    why: "raise the page only on a tick that actually measured a canary. Containment is measured only when a worker task wants dispatching, so a quiet tick runs none -- and `announceable` reads an escalation absent from a complete tick as resolved, so the next quiet tick announces CLEARED for a sandbox nobody re-measured. The backup path carries its failures on ctx for this exact reason",
    test: "test/canary-page.test.mjs",
    expectRed: "the page still stands on a second tick",
    edits: [{ file: "src/daemon.mjs",
              find: "  if (ctx.canaryFailure) raise(ctx.canaryFailure);",
              replace: "  if (canary && ctx.canaryFailure) raise(ctx.canaryFailure);" }],
  },
  {
    name: "canary-page-clears-when-one-passes",
    why: "never clear the standing page. It then becomes a latch that a human can only silence by restarting the daemon, and an operator who is never told a fault is over cannot tell `resolved` from `reeve stopped looking` -- which is the reason `announceable` announces clearing at all",
    test: "test/canary-page.test.mjs",
    expectRed: "a canary that passes afterwards clears the page",
    edits: [{ file: "src/daemon.mjs",
              find: "  if (canary?.ok) ctx.canaryFailure = null;",
              replace: "  if (false) ctx.canaryFailure = null;" }],
  },
  {
    name: "canary-page-ignores-a-skipped-canary",
    why: "treat a SKIPPED canary as a failed one. `skipped` means containment was already open for a cheaper reason so the canary never ran at all, and paging the sandbox identity for it sends the founder to the policy when the fault is a credential in the login keychain",
    test: "test/canary-page.test.mjs",
    expectRed: "a SKIPPED canary raises no page",
    edits: [{ file: "src/daemon.mjs",
              find: "  else if (canary && !canary.skipped && canary.id != null) {",
              replace: "  else if (canary && canary.id != null) {" }],
  },
  {
    name: "canary-page-needs-a-canary-that-ran",
    why: "page when there was no canary to run. A null id means no CLI version or sandbox block existed to run one under -- a configuration fault, reported by the generic containment identity -- and calling it a failed canary claims the sandbox stopped containing a worker when nothing measured whether it does",
    test: "test/canary-page.test.mjs",
    expectRed: "a canary with no id raises no page",
    edits: [{ file: "src/daemon.mjs",
              find: "  else if (canary && !canary.skipped && canary.id != null) {",
              replace: "  else if (canary && !canary.skipped) {" }],
  },
  {
