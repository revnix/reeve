...
`,
  ...
  },
  {
    ...
    edits: [{ file: "src/paths.mjs",
              find: "        const stranded = moved.reverse().filter((suffix) => { try { rename(next + suffix, legacy + suffix); return false; } catch { return true; } });\n        if (stranded.length) throw new Error(`${e.message}; and ${stranded.map((s) => next + s).join(\", \")} could not be moved back`);\n",
              replace: "" }],
  },
  {
  ...
  },
  {
    ...
    test: "test/init-store.test.mjs",
    expectRed: "while another process holds the move, this one waits and uses what it made, moving nothing itself",
    edits: [{ file: "src/paths.mjs",
              find: "  try { mkdirSync(dirname(next), { recursive: true }); held = takeLock(lock, alive); }",
              replace: "  try { mkdirSync(dirname(next), { recursive: true }); held = true; }" }],
  },
  {
    name: "legacy-move-takes-over-a-dead-lock",
    why: "never take over a lock. One left by a move whose process died then blocks the store for good",
    test: "test/init-store.test.mjs",
    expectRed: "a lock left by a process that died is taken over, and the move finishes",
    edits: [{ file: "src/paths.mjs",
              find: "      if (Number.isInteger(holder) && holder > 0 && alive(holder)) return false;",
              replace: "      return false;" }],
  },
  {
    name: "legacy-move-releases-its-lock",
    why: "keep the lock after the move. Every later command then waits on a lock nobody holds",
    test: "test/init-store.test.mjs",
    expectRed: "the lock is let go once the move is done",
    edits: [{ file: "src/paths.mjs",
              find: "  finally { rmSync(lock, { force: true }); }\n",
              replace: "" }],
  },
  {
    ...
    test: "test/init-store.test.mjs",
    expectRed: "a move a live process never finishes fails loudly, and touches nothing",
    edits: [{ file: "src/paths.mjs",
              find: "      throw new Error(`the state store at ${legacy} is being moved to ${next} by another process; try again once it finishes`);",
